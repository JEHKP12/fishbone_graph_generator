...
def problems(ax, label, p_x, p_y, angle_y, bone_length=180, scale=0.55):
    # Fixed angle for arrows
    FIXED_ANGLE_DEGREES = 52 if angle_y > 0 else -52
    ARROW_LENGTH = bone_length  # Fixed length for the arrow
    
    # Calculate the text box position using the fixed angle
    ...
    ax.annotate(label.upper(), xy=(p_x, p_y), xytext=(x_off, y_off),
                ...
                xycoords='data', textcoords='offset points',  # Changed to offset points for better control
                ha='center', va='center',
...
def draw_body(ax, categories, head_radius, problem_fontsize, main_problem, bone_length=180):
    ...
    length  = math.ceil(n_cat / 2) - 1          # how many "segments"
    x_tail  = tail_offset(n_cat) - length       # ← NEW
    x_head  = 2 + length

    draw_spine(ax, x_tail, x_head, head_radius, main_problem, problem_fontsize)                  # <─ call unchanged helper

    offset      = 0
    anchor_xy   = [1.55, 0.5]  # Increased distance between problem and cause arrows
    ...
    for idx, (cat, sub) in enumerate(categories.items()):
        ...
        p_x = anchor_xy[0] + length + offset    # category arrow start
        c_x = anchor_xy[1] + length + offset    # causes   arrow start
        if not upper:
            offset -= BONE_SPACING              # keep your spacing rule

        problems(ax, cat, p_x, 0, angle_y, bone_length)          # existing helper
        causes(ax, sub, c_x, cause_y, top=upper)    # existing helper
    ...
    if st.button("Generate Preview"):
        try:
            ...
            if 'Main Problem' not in df.columns:
                raise ValueError("Column 'Main Problem' is not found in Excel.")
        ...
        ax.set_xlim(x_tail - 2, x_head + head_radius + 2)  # Dynamic xlim
        ax.set_ylim(-6, 6)  # Keep ylim fixed for now
        ax.axis('off')
